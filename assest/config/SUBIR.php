...
class SUBIR {
    ...
    public function subirImg($ARCHIVO,$NOMBRENUEVOIMG, $DIRECTORIODESTINON){        
        ...
        if (isset($ARCHIVORIGEN) && $ARCHIVORIGEN != "") {
            ...
            $tiposPermitidos = ['image/jpeg', 'image/gif', 'image/jpg', 'image/png'];
            $tipoValido = in_array($tipo, $tiposPermitidos, true);
            $tamanoValido = ($tamano > 0 && $tamano <= $TAMANOMAXIMO);
            ...
            if (!$tipoValido || !$tamanoValido) {
                ...
                if (strpos($tipo, "gif") !== false ){      
                    $FORMATO=".gif";                
                } elseif (strpos($tipo, "jpeg") !== false){
                    $FORMATO=".png";
                    
                } elseif (strpos($tipo, "jpg") !== false){
                    $FORMATO=".png";
                ...
                if (move_uploaded_file($temp,$DIRECTORIODESTINO.$NUEVONOMBRE.$FORMATO)) {
                    ...
                    chmod($DIRECTORIODESTINO.$NUEVONOMBRE.$FORMATO, 0644);
