...
class DESPACHOEX_ADO
{
    ...
    public function listarDetalladoExportacionDesdeVista($NOMBRE_TEMPORADA = null, $NOMBRE_EMPRESA = null)
    {
        try {
            $sql = "SELECT * FROM view_detalladoexp WHERE 1=1";
            $params = [];

            if (!empty($NOMBRE_TEMPORADA)) {
                $sql .= " AND `Temporada` = :temporada";
                $params[':temporada'] = $NOMBRE_TEMPORADA;
            }

            if (!empty($NOMBRE_EMPRESA)) {
                $sql .= " AND `Empresa` = :empresa";
                $params[':empresa'] = $NOMBRE_EMPRESA;
            }

            $sql .= " ORDER BY STR_TO_DATE(`Fecha Despacho`, '%d-%m-%Y') DESC, `Número Despacho` DESC";
