...
// Notice: Undefined variable: PCDESPACHOMP in /Users/mike/Sites/localhost/web-control/fruta/vista/registroSelecionPCProcesoMP.php on line 312
    ...
    public function listarExistenciaGeneralProductor($EMPRESA, $PLANTA, $TEMPORADA, $PRODUCTOR)
    {
        try {
            $sql = "SELECT
                        ID_EXIMATERIAPRIMA,
                        FOLIO_EXIMATERIAPRIMA,
                        IFNULL(DATE_FORMAT(FECHA_COSECHA_EXIMATERIAPRIMA, '%d-%m-%Y'), 'Sin Datos') AS FECHA_COSECHA,
                        IFNULL(CANTIDAD_ENVASE_EXIMATERIAPRIMA, 0) AS ENVASE,
                        IFNULL(KILOS_NETO_EXIMATERIAPRIMA, 0) AS NETO,
                    ...
                    WHERE ESTADO_REGISTRO = 1
                      ...
                      AND ID_TEMPORADA = ?
                      AND ID_PRODUCTOR = ?
                    ORDER BY ID_EXIMATERIAPRIMA DESC";
            $datos = $this->conexion->prepare($sql);
            $datos->execute(array($EMPRESA, $PLANTA, $TEMPORADA, $PRODUCTOR));
            $resultado = $datos->fetchAll(PDO::FETCH_ASSOC);
            $datos = null;
