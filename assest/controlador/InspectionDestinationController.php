...
class InspectionDestinationController
{
    private $conexion;
    ...
    public function getCatalogs($empresaId = null, $plantaId = null, $temporadaId = null, $includeReceptionId = null)
    {
        ...
        if ($empresaId && $plantaId && $temporadaId) {
            ...
            if ($this->tableExists('inspections') && $this->hasColumn('inspections', 'reception_id')) {
                $excludeInspected = " AND d.ID_DESPACHOEX NOT IN (
                        SELECT reception_id
                        FROM inspections
                        WHERE reception_id IS NOT NULL" . ($useSourceType ? " AND source_type = 'DESTINO'" : "") . (!empty($includeReceptionId) ? " AND reception_id <> :include_reception_id" : "") . "
                    ) ";
            }

            $stmt = $this->conexion->prepare(
                "SELECT d.ID_DESPACHOEX id,
                        d.NUMERO_CONTENEDOR_DESPACHOEX reception_number,
                        d.FECHA_DESPACHOEX reception_date,
                        '' reception_hour,
                        '' guide_number,
                        IFNULL(d.KILOS_NETO_DESPACHOEX,0) net_kilos,
                        IFNULL(d.KILOS_BRUTO_DESPACHOEX,0) gross_kilos,
                        IFNULL(d.CANTIDAD_ENVASE_DESPACHOEX,0) package_count,
                        MAX(exi.ID_PRODUCTOR) producer_id,
                        IFNULL(pr.NOMBRE_PRODUCTOR, 'Sin datos') producer_name,
                        GROUP_CONCAT(DISTINCT exi.ID_VESPECIES) variety_ids,
                        GROUP_CONCAT(DISTINCT v.NOMBRE_VESPECIES ORDER BY v.NOMBRE_VESPECIES SEPARATOR ', ') varieties_label,
                        d.FECHA_DESPACHOEX AS FECHA,
                        WEEK(d.FECHA_DESPACHOEX,3) AS SEMANA,
                        WEEKOFYEAR(d.FECHA_DESPACHOEX) AS SEMANAISO
                 FROM fruta_despachoex d
                 LEFT JOIN fruta_exiexportacion exi ON exi.ID_DESPACHOEX = d.ID_DESPACHOEX AND exi.ESTADO_REGISTRO = 1
                 LEFT JOIN fruta_productor pr ON pr.ID_PRODUCTOR = exi.ID_PRODUCTOR
                 LEFT JOIN fruta_vespecies v ON v.ID_VESPECIES = exi.ID_VESPECIES
                 WHERE d.ESTADO_REGISTRO = 1
                   AND d.ESTADO = 1
                   AND d.ID_EMPRESA = :empresa
                   AND d.ID_PLANTA = :planta
                   AND d.ID_TEMPORADA = :temporada
                   {$excludeInspected}
                 GROUP BY d.ID_DESPACHOEX
                 ORDER BY d.ID_DESPACHOEX DESC"
            );
            $params = [
        ...
        if ($empresaId && $plantaId && $temporadaId && $this->tableExists('fruta_exiexportacion')) {
            $folioStmt = $this->conexion->prepare(
                "SELECT exi.ID_DESPACHOEX reception_id,
                 ...
                 WHERE exi.ESTADO_REGISTRO = 1
                   ...
                   AND exi.ID_DESPACHOEX IS NOT NULL
    ...
    {
        if (!$this->tableExists('fruta_despachoex')) {
            return null;
        }
        try {
            $rxStmt = $this->conexion->prepare(
                "SELECT ID_EMPRESA, ID_PLANTA FROM fruta_despachoex WHERE ID_DESPACHOEX = :id LIMIT 1"
            );
    ...
    {
        $baseDir = __DIR__ . '/../../calidad/inspection-destination/uploads';
        if (!is_dir($baseDir)) {
        ...
        foreach ($photosMeta as $slot => $meta) {
            ...
            $relativePath = 'calidad/inspection-destination/uploads/inspection_' . (int)$inspectionId . '/' . $finalName;
            $stmt->execute([
    ...
    public function getInspectionGrouped($empresaId = null, $plantaId = null, $temporadaId = null)
    {
        ...
        $receptionJoin = $this->hasColumn('inspections', 'reception_id')
            ? "LEFT JOIN fruta_despachoex rx ON rx.ID_DESPACHOEX = i.reception_id"
            : "";
        $receptionNumberExpr = $this->hasColumn('inspections', 'reception_id')
            ? "IFNULL(rx.NUMERO_CONTENEDOR_DESPACHOEX, 'Sin contenedor')"
            : "'Sin contenedor'";
        $guideNumberExpr = $this->hasColumn('inspections', 'reception_id')
            ? "IFNULL(rx.FECHA_DESPACHOEX, 'Sin fecha')"
            : "'Sin guía'";
    ...
    public function getInspectionSummaryById($inspectionId)
    {
        ...
        $receptionNumberExpr = $this->hasColumn('inspections', 'reception_id')
            ? "IFNULL(rx.NUMERO_CONTENEDOR_DESPACHOEX, 'Sin contenedor')"
            : "'Sin contenedor'";
        $sourceFilter = $this->hasColumn('inspections', 'source_type') ? " AND i.source_type = 'DESTINO'" : "";
    ...
    public function getInspectionForEdit($inspectionId)
    {
        ...
        $whereSource = $this->hasColumn('inspections', 'source_type') ? " AND i.source_type = 'DESTINO'" : "";
