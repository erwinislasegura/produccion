...
class InspectionMPController
{
    ...
    public function getCatalogs($empresaId = null, $plantaId = null, $temporadaId = null, $includeReceptionId = null)
    {
        ...
        if ($empresaId && $plantaId && $temporadaId) {
            ...
            if ($this->tableExists('inspections') && $this->hasColumn('inspections', 'reception_id')) {
                $excludeInspected = " AND r.ID_RECEPCION NOT IN (
                        SELECT reception_id
                        FROM inspections
                        WHERE reception_id IS NOT NULL" . ($useSourceType ? " AND source_type = 'MP'" : "") . (!empty($includeReceptionId) ? " AND reception_id <> :include_reception_id" : "") . "
                    ) ";
            }
            ...
            $stmt = $this->conexion->prepare(
                "SELECT *,
                        r.ID_RECEPCION id,
                        r.NUMERO_RECEPCION reception_number,
                        r.FECHA_RECEPCION reception_date,
                        r.HORA_RECEPCION reception_hour,
                        r.NUMERO_GUIA_RECEPCION guide_number,
                        IFNULL(r.KILOS_NETO_RECEPCION,0) net_kilos,
                        IFNULL(r.KILOS_BRUTO_RECEPCION,0) gross_kilos,
                        IFNULL(r.CANTIDAD_ENVASE_RECEPCION,0) package_count,
                        r.ID_PRODUCTOR producer_id,
                        IFNULL(pr.NOMBRE_PRODUCTOR, 'Sin datos') producer_name,
                        GROUP_CONCAT(DISTINCT dv.ID_VESPECIES) variety_ids,
                        GROUP_CONCAT(DISTINCT v.NOMBRE_VESPECIES ORDER BY v.NOMBRE_VESPECIES SEPARATOR ', ') varieties_label,
                 ...
                 LEFT JOIN fruta_productor pr ON pr.ID_PRODUCTOR = r.ID_PRODUCTOR
                 LEFT JOIN fruta_drecepcionmp dv ON dv.ID_RECEPCION = r.ID_RECEPCION AND dv.ESTADO_REGISTRO = 1
                 LEFT JOIN fruta_vespecies v ON v.ID_VESPECIES = dv.ID_VESPECIES
        ...
        $foliosByReception = [];
        if ($empresaId && $plantaId && $temporadaId && $this->tableExists('fruta_eximateriaprima')) {
            $folioStmt = $this->conexion->prepare(
                "SELECT exi.ID_RECEPCION reception_id,
                        exi.ID_EXIMATERIAPRIMA folio_id,
                        IFNULL(exi.FOLIO_EXIMATERIAPRIMA, exi.ID_EXIMATERIAPRIMA) folio,
                        IFNULL(exi.ALIAS_DINAMICO_FOLIO_EXIMATERIAPRIMA, '') alias_folio,
                        IFNULL(exi.ALIAS_ESTATICO_FOLIO_EXIMATERIAPRIMA, '') final_folio,
                        IFNULL(exi.CANTIDAD_ENVASE_EXIMATERIAPRIMA, 0) sample_size,
                        IFNULL(v.NOMBRE_VESPECIES, 'Sin variedad') variety_name
                 FROM fruta_eximateriaprima exi
                 LEFT JOIN fruta_drecepcionmp dv ON dv.ID_RECEPCION = exi.ID_RECEPCION
                    AND dv.ESTADO_REGISTRO = 1
                    AND dv.ID_VESPECIES = exi.ID_VESPECIES
                 LEFT JOIN fruta_vespecies v ON v.ID_VESPECIES = IFNULL(dv.ID_VESPECIES, exi.ID_VESPECIES)
                 WHERE exi.ESTADO_REGISTRO = 1
                   AND exi.ID_EMPRESA = :empresa
                   AND exi.ID_PLANTA = :planta
                   AND exi.ID_TEMPORADA = :temporada
                   AND exi.ID_RECEPCION IS NOT NULL
                 ORDER BY exi.ID_EXIMATERIAPRIMA ASC"
            );
            $folioStmt->execute([
                ':empresa' => (int)$empresaId,
                ':planta' => (int)$plantaId,
        ...
        try {
            ...
            foreach ($payload['details'] as $detailIndex => $detail) {
                ...
                if ($folioSource && !empty($folioSource['variety_id_source'])) {
                    $varietyId = $this->resolveVarietyIdForInspection(
                        (int)$folioSource['variety_id_source'],
                        (string)($folioSource['variety_name'] ?? '')
                    );
                } elseif ($varietyId > 0) {
                    $varietyId = $this->resolveVarietyIdForInspection($varietyId, (string)($folioSource['variety_name'] ?? ''));
                }
                if ($varietyId <= 0 || !$this->recordExistsByColumn('varieties', 'id', $varietyId)) {
    ...
    {
        if (!$this->tableExists('fruta_recepcionmp')) {
            return null;
        }
        try {
            $rxStmt = $this->conexion->prepare(
                "SELECT ID_EMPRESA, ID_PLANTA FROM fruta_recepcionmp WHERE ID_RECEPCION = :id LIMIT 1"
            );
    ...
    public function getInspectionGrouped($empresaId = null, $plantaId = null, $temporadaId = null)
    {
        ...
        $receptionJoin = $this->hasColumn('inspections', 'reception_id')
            ? "LEFT JOIN fruta_recepcionmp rx ON rx.ID_RECEPCION = i.reception_id"
            : "";
        $receptionNumberExpr = $this->hasColumn('inspections', 'reception_id')
            ? "IFNULL(rx.NUMERO_RECEPCION, 'Sin recepción')"
            : "'Sin recepción'";
        $guideNumberExpr = $this->hasColumn('inspections', 'reception_id')
            ? "IFNULL(rx.NUMERO_GUIA_RECEPCION, 'Sin guía')"
            : "'Sin guía'";
    ...
    {
        if (!$this->tableExists('fruta_eximateriaprima')) {
            return null;
        }
        $stmt = $this->conexion->prepare(
            "SELECT exi.ID_EXIMATERIAPRIMA folio_id,
                    IFNULL(exi.CANTIDAD_ENVASE_EXIMATERIAPRIMA, 0) sample_size,
                    IFNULL(dv.ID_VESPECIES, exi.ID_VESPECIES) variety_id_source,
             ...
             LEFT JOIN fruta_drecepcionmp dv ON dv.ID_RECEPCION = exi.ID_RECEPCION
                ...
                AND dv.ID_VESPECIES = exi.ID_VESPECIES
             LEFT JOIN fruta_vespecies v ON v.ID_VESPECIES = IFNULL(dv.ID_VESPECIES, exi.ID_VESPECIES)
             WHERE exi.ID_EXIMATERIAPRIMA = :folio
             LIMIT 1"
        );
