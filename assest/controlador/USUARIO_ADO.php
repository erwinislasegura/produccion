...
class USUARIO_ADO {
    ...
    public function BuscarUsuarioNombre($NOMBRE){
        try{
            ...
            $nombreLike = "%".$NOMBRE."%";
            $datos->bindParam(':nombre', $nombreLike, PDO::PARAM_STR);
            $datos->execute();
            $resultado = $datos->fetchAll();
            $datos=null;
    ...
    public function iniciarSession($NOMBRE,$CONTRASENA){
        try{
            $datos=$this->conexion->prepare("SELECT * FROM usuario_usuario
                                            ...
                                            AND (
                                                CONTRASENA_USUARIO = SHA2(:contrasena_hash, 512)
                                                OR CONTRASENA_USUARIO = :contrasena_legacy
                                            );");
            $datos->bindParam(':nombre', $NOMBRE, PDO::PARAM_STR);
            $datos->bindParam(':contrasena_hash', $CONTRASENA, PDO::PARAM_STR);
            $datos->bindParam(':contrasena_legacy', $CONTRASENA, PDO::PARAM_STR);
            $datos->execute();
            $resultado = $datos->fetchAll();
            $datos=null;
