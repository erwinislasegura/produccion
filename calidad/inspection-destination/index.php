...
if (isset($_POST['GUARDAR']) || isset($_POST['GUARDAR_CERRAR'])) {
    ...
    try {
        if (!is_array($payload)) {
            ...
            throw new InvalidArgumentException('No se pudo construir el formulario para guardar (' . $decodeError . '). Seleccione contenedor y vuelva a intentar.');
        }
...
<html lang="es">
<head>
    <title>Inspección en Destino</title>
    ...
    <style>
        ...
        .inspection-grid-card { border: 1px solid #e5e7eb; border-radius: 8px; padding: 10px; background: #fff; margin-bottom: 10px; }
        .section-title { font-size: 13px; font-weight: 700; color: #1f2937; margin-bottom: 8px; border-left: 3px solid #374151; padding-left: 8px; }
        ...
        .producer-card { border: 1px solid #e7eaef; border-radius: 6px; background: #f7f8fa; padding: 8px; }
        ...
        .modal-photo-header { background: #fff; border-bottom:1px solid #e5e7eb; }
        .modal-photo-body { max-height: 62vh; overflow-y: auto; }
        ...
        .folio-detail-body { max-height: 68vh; overflow-y: auto; }
    ...
    <div class="content-wrapper">
        <div class="container-full">
            <div class="content-header">
                <div class="d-flex align-items-center"><div class="mr-auto"><h3 class="page-title">Calidad - Inspección en Destino</h3></div><?php include_once '../../assest/config/verIndicadorEconomico.php'; ?></div>
            </div>
            <section class="content">
                <div class="box">
                    <div class="box-header with-border"><h4 class="box-title">Formulario Inspección en Destino</h4></div>
                    <form data-form-layout="oneline-1" style="display:flex;flex-wrap:wrap;align-items:flex-end;gap:0.65rem;padding:0.65rem 0;border-bottom:1px solid #3a3a3a;" method="post" id="inspection-form" class="inspection-compact form-one-line" enctype="multipart/form-data">
                        ...
                        <div class="box-body">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label>N° contenedor despacho exportación <span class="inspection-chip">Origen</span></label>
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control readonly-field" id="received_count" value="0" readonly>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                ...
                                <div class="col-md-6">
                                    <div class="inspection-grid-card" style="height:100%;">
                                        <div class="d-flex justify-content-between align-items-center mb-8">
                                            <div class="section-title mb-0">2) Datos del despacho de exportación</div>
                                        </div>
                                        <div class="producer-card">
                                            <div class="producer-grid">
                                                <div class="producer-item"><small>N° Despacho</small><b id="rx_number">-</b></div>
                                        ...
                                        <div class="producer-card">
                                            <div id="producer_details" class="producer-grid">
                                                <div class="producer-item">
                                                    ...
                                                    <b>Sin datos de productor para el contenedor seleccionado.</b>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div class="row">
                                <div class="col-12">
                                    ...
                                    <div class="mb-10">
                                        <label>Detalle por tandas (folios del contenedor)</label>
                                        <div class="table-responsive">
                                            <table class="table table-sm table-bordered" id="folios_defects_table">
                                                <thead></thead>
                                                <tbody>
                                                    <tr>
                                                        <td class="text-muted text-center">Seleccione un contenedor con folios para completar los defectos.</td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
...
function fillReceptionSelect() {
    ...
    if (editingInspection && editingInspection.header) {
        ...
        if (editReceptionId && !Array.from(document.getElementById('reception_id').options).some((opt) => String(opt.value) === editReceptionId)) {
            ...
            option.textContent = `${header.reception_number || editReceptionId} · ${header.inspection_date || 'Sin fecha'} · ${header.producer_name || 'Contenedor edición'}`;
...
function renderProducerDetails(rx) {
    ...
    if (!producer) {
        detailsNode.innerHTML = '<div class="producer-item"><small>Productor</small><b>Sin datos para el contenedor seleccionado.</b></div>';
        return;
    }
    const fields = [
...
function renderFoliosDefectsTable() {
    ...
    if (!folios.length) {
        thead.innerHTML = '';
        const message = rid
            ? 'Sin folios activos para el contenedor seleccionado.'
            : 'Seleccione un contenedor para cargar folios y datos dependientes.';
...
function updateReceptionInfo() {
    ...
    document.getElementById('received_count').value = rx ? String(rx.package_count || 0) : '0';
...
document.getElementById('inspection-form').addEventListener('submit', function (event) {
    ...
    if (!document.getElementById('reception_id').value) {
        Swal.fire({ icon: 'warning', title: 'Contenedor requerido', text: 'Debe seleccionar un contenedor para guardar la inspección.' });
        event.preventDefault();
        return false;
    }
    if (!document.getElementById('producer_id').value) {
        Swal.fire({ icon: 'warning', title: 'Productor requerido', text: 'No fue posible resolver el productor del contenedor seleccionado.' });
        event.preventDefault();
        return false;
    }
    ...
    if (!payload.details.length) {
        Swal.fire({ icon: 'warning', title: 'Sin detalle', text: 'El contenedor seleccionado no tiene folios para guardar.' });
        event.preventDefault();
        return false;
    }
