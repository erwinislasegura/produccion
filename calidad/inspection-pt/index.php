...
if (isset($_POST['GUARDAR']) || isset($_POST['GUARDAR_CERRAR'])) {
    ...
    try {
        if (!is_array($payload)) {
            ...
            throw new InvalidArgumentException('No se pudo construir el formulario para guardar (' . $decodeError . '). Seleccione proceso y vuelva a intentar.');
        }
    ...
    <div class="content-wrapper">
        <div class="container-full">
            <div class="content-header">
                <div class="d-flex align-items-center"><div class="mr-auto"><h3 class="page-title">Calidad - Inspección PT</h3></div><?php include_once '../../assest/config/verIndicadorEconomico.php'; ?></div>
            </div>
            <section class="content">
                <div class="box">
                    <div class="box-header with-border"><h4 class="box-title">Formulario Inspección PT</h4></div>
                        ...
                        <div class="box-body">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label>Proceso <span class="inspection-chip">Origen</span></label>
                                ...
                                <div class="col-md-6">
                                    <div class="inspection-grid-card" style="height:100%;">
                                        <div class="d-flex justify-content-between align-items-center mb-8">
                                            <div class="section-title mb-0">2) Datos del proceso</div>
                                        </div>
                                        <div class="producer-card">
                                            <div class="producer-grid">
                                                <div class="producer-item"><small>N° Proceso</small><b id="rx_number">-</b></div>
                                        ...
                                        <div class="producer-card">
                                            <div id="producer_details" class="producer-grid">
                                                <div class="producer-item">
                                                    ...
                                                    <b>Sin datos de productor para la proceso seleccionada.</b>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div class="row">
                                <div class="col-12">
                                    ...
                                    <div class="mb-10">
                                        <label>Detalle por tandas (folios de proceso)</label>
                                        <div class="table-responsive">
                                            <table class="table table-sm table-bordered" id="folios_defects_table">
                                                <thead></thead>
                                                <tbody>
                                                    <tr>
                                                        <td class="text-muted text-center">Seleccione una proceso con folios para completar los defectos.</td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
...
function fillReceptionSelect() {
    ...
    if (editingInspection && editingInspection.header) {
        ...
        if (editReceptionId && !Array.from(document.getElementById('reception_id').options).some((opt) => String(opt.value) === editReceptionId)) {
            ...
            option.textContent = `${header.reception_number || editReceptionId} · ${header.inspection_date || 'Sin fecha'} · ${header.producer_name || 'Proceso edición'}`;
...
function renderProducerDetails(rx) {
    ...
    if (!producer) {
        detailsNode.innerHTML = '<div class="producer-item"><small>Productor</small><b>Sin datos para la proceso seleccionada.</b></div>';
        return;
    }
    const fields = [
...
function renderFoliosDefectsTable() {
    ...
    if (!folios.length) {
        thead.innerHTML = '';
        const message = rid
            ? 'Sin folios activos para la proceso seleccionada.'
            : 'Seleccione una proceso para cargar folios y datos dependientes.';
...
document.getElementById('inspection-form').addEventListener('submit', function (event) {
    ...
    if (!document.getElementById('reception_id').value) {
        Swal.fire({ icon: 'warning', title: 'Proceso requerida', text: 'Debe seleccionar una proceso para guardar la inspección.' });
        event.preventDefault();
        return false;
    }
    if (!document.getElementById('producer_id').value) {
        Swal.fire({ icon: 'warning', title: 'Productor requerido', text: 'No fue posible resolver el productor de la proceso seleccionada.' });
        event.preventDefault();
        return false;
    }
    ...
    if (!payload.details.length) {
        Swal.fire({ icon: 'warning', title: 'Sin detalle', text: 'La proceso seleccionada no tiene folios para guardar.' });
        event.preventDefault();
        return false;
    }
