...
$conexion->exec("CREATE TABLE IF NOT EXISTS calidad_defectos_condicion (id INT AUTO_INCREMENT PRIMARY KEY,name VARCHAR(120) NOT NULL,defect_type ENUM('critical','major','minor') NOT NULL DEFAULT 'minor',is_active TINYINT(1) NOT NULL DEFAULT 1,defect_id INT NULL,created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP) ENGINE=InnoDB");
...
$name=''; $type='minor';
if ($accion==='0' && $id>0) {
  $row=$conexion->prepare("SELECT defect_id FROM calidad_defectos_condicion WHERE id=:id");$row->execute([':id'=>$id]);$x=$row->fetch(PDO::FETCH_ASSOC);
  $conexion->prepare("UPDATE calidad_defectos_condicion SET is_active=0 WHERE id=:id")->execute([':id'=>$id]);
  if($x && !empty($x['defect_id'])){$conexion->prepare("UPDATE defects SET is_active=0 WHERE id=:id")->execute([':id'=>(int)$x['defect_id']]);}
  header('Location: mantenedorInspectionMpDefectosCondicion.php');exit;
}
if($id>0){$st=$conexion->prepare("SELECT * FROM calidad_defectos_condicion WHERE id=:id LIMIT 1");$st->execute([':id'=>$id]);$r=$st->fetch(PDO::FETCH_ASSOC);if($r){$name=$r['name'];$type=$r['defect_type'];}}
if(isset($_POST['GUARDAR'])){
  $idForm=(int)($_POST['id']??0); $name=trim($_POST['name']??''); $type=trim($_POST['defect_type']??'minor');
  if($name!==''){
    if($idForm>0){
      $conexion->prepare("UPDATE calidad_defectos_condicion SET name=:name, defect_type=:type, is_active=1 WHERE id=:id")->execute([':name'=>$name,':type'=>$type,':id'=>$idForm]);
    } else {
      $masterType = (!$supportsCritical && $type==='critical') ? 'major' : $type;
      if($hasScope){$conexion->prepare("INSERT INTO defects (name, defect_type, defect_scope, is_active) VALUES (:name,:type,'condition',1)")->execute([':name'=>$name,':type'=>$masterType]);}
      ...
      $conexion->prepare("INSERT INTO calidad_defectos_condicion (name, defect_type, is_active, defect_id) VALUES (:name,:type,1,:defect_id)")->execute([':name'=>$name,':type'=>$type,':defect_id'=>$masterId]);
    }
  }
  header('Location: mantenedorInspectionMpDefectosCondicion.php');exit;
}
$rows=$conexion->query("SELECT id,name,defect_type,is_active FROM calidad_defectos_condicion ORDER BY id DESC")->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html lang="es">
<head>
    <title>Defectos de Condición</title>
            ...
            <div class="container-full">
                <div class="content-header">
                    <div class="d-flex align-items-center">
                        <div class="mr-auto">
                            <h3 class="page-title">Mantenedor - Defectos de Condición</h3>
                        </div>
                            ...
                            <div class="box mp-panel">
                                ...
                                <div class="box-body">
                                    <form class="form-one-line" data-form-layout="oneline-1" style="display:flex;flex-wrap:wrap;align-items:flex-end;gap:0.65rem;padding:0.65rem 0;border-bottom:1px solid #3a3a3a;" method="post">
                                        ...
                                        <a class="btn mp-btn-light" href="mantenedorInspectionMpDefectosCondicion.php">Nuevo</a>
                                    </form>
                                </div>
                            </div>
                        </div>
