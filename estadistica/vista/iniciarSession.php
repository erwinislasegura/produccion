...
<head>
    <meta charset="UTF-8">
    <title>Portal Estadísticas - SmartBerry ONE</title>
...
<main class="page">
    <section class="left-panel">
        <div class="left-content">
            <header class="brand">
                ...
                <div class="module-badge"><span class="material-icons-round">query_stats</span>Portal estadísticas</div>
                <h1 class="title">Acceso a indicadores SmartBerry ONE</h1>
                <p class="subtitle">Ingresa para consultar reportes, indicadores productivos y datos de gestión.</p>
            ...
            <section class="login-card">
                ...
                <form method="post" id="loginForm" autocomplete="on">
                    <div class="form-grid">
                        ...
                        <div class="form-group"><label class="form-label" for="CONTRASENA">Contraseña</label><input type="password" class="form-input" id="CONTRASENA" name="CONTRASENA" placeholder="Contraseña" value="<?= e($CONTRASENA) ?>" autocomplete="current-password" required></div>
                        <div class="form-group"><label class="form-label" for="EMPRESA">Empresa</label><select class="form-input" id="EMPRESA" name="EMPRESA" required><option value="">Seleccione empresa</option><?php foreach ($ARRAYEMPRESA as $r): ?><option value="<?= e($r['ID_EMPRESA']) ?>" <?= $EMPRESA == $r['ID_EMPRESA'] ? "selected" : "" ?>><?= e($r['NOMBRE_EMPRESA']) ?></option><?php endforeach; ?></select></div>
                        <div class="form-group"><label class="form-label" for="ESPECIE">Especie</label><select class="form-input" id="ESPECIE" name="ESPECIE" required><option value="">Seleccione especie</option><option value="1">Arándanos</option><option value="3">Espárragos</option></select></div>
                        <div class="form-group"><label class="form-label" for="TEMPORADA">Temporada</label><select class="form-input" id="TEMPORADA" name="TEMPORADA" required><?php foreach ($ARRAYTEMPORADA as $r): ?><option value="<?= e($r['ID_TEMPORADA']) ?>" <?= $TEMPORADA == $r['ID_TEMPORADA'] ? "selected" : "" ?>><?= e($r['NOMBRE_TEMPORADA']) ?></option><?php endforeach; ?></select></div>
                        <div class="option-box"><input type="checkbox" id="PRECARGA_EXPORTACION_ESTADISTICA"><label for="PRECARGA_EXPORTACION_ESTADISTICA">Cargar información pesada en segundo plano.</label></div>
                        <div class="security-note"><span class="material-icons-round">lock</span>Acceso privado para usuarios autorizados</div>
                        <div class="btn-row"><a href="../../" class="btn btn-back">Volver</a><button type="submit" class="btn btn-login" name="ENTRAR">Entrar</button></div>
                    </div>
                </form>
            </section>
            <div class="footer-text">SmartBerry ONE · Portal privado de estadísticas.</div>
        </div>
    </section>
    <section class="right-panel">
        ...
        <div class="right-info"><div class="right-kicker"><span class="material-icons-round">analytics</span>Gestión de datos</div><h2>Reportes claros para decisiones operativas</h2><p>Accede a información productiva, temporadas, especies y empresas desde una vista ordenada, segura y conectada con los procesos internos.</p><div class="right-tags"><div class="right-tag"><span class="material-icons-round">bar_chart</span>Reportes</div><div class="right-tag"><span class="material-icons-round">query_stats</span>Indicadores</div><div class="right-tag"><span class="material-icons-round">eco</span>Producción</div><div class="right-tag"><span class="material-icons-round">shield</span>Control</div></div></div>
    </section>
</main>
<script>
    ...
    if (loginForm) { loginForm.addEventListener('submit', () => { const precarga = document.getElementById('PRECARGA_EXPORTACION_ESTADISTICA')?.checked; if (precarga) { localStorage.setItem('prefetch_despachoex_exportacion', '1'); } else { localStorage.removeItem('prefetch_despachoex_exportacion'); } }); }
</script>
<?php if ($detect->isMobile() && $detect->isiOS()): ?><script>Swal.fire({icon:"info",title:"iPhone detectado",html:"Algunas vistas no están adaptadas 📱<br>Recomendamos usar tablet Android/iPad o PC",confirmButtonText:"Vale! 😉"});</script><?php endif; ?>
<?php if ($detect->isMobile() && $detect->isAndroidOS()): ?><script>Swal.fire({icon:"info",title:"Android detectado",html:"Algunas vistas no están adaptadas 🤖<br>Recomendamos usar tablet Android/iPad o PC",confirmButtonText:"Vale! 😉"});</script><?php endif; ?>
<?php
// === PROCESO DE LOGIN ===
