...
$NOMBRESMERCADOSPRODUCTOR = array();
$NOMBRESMERCADOSESTANDAR = array();
...
if ((int)$PRODUCTOR > 0 && (int)$ESTANDAR > 0) {
    ...
    foreach ($MERCADOSPRODUCTORACTUAL as $IDM) {
        if (isset($LISTAMERCADOS[$IDM])) {
            $NOMBRESMERCADOSPRODUCTOR[] = $LISTAMERCADOS[$IDM];
        }
    }
    foreach ($MERCADOSESTANDARACTUAL as $IDM) {
        if (isset($LISTAMERCADOS[$IDM])) {
            $NOMBRESMERCADOSESTANDAR[] = $LISTAMERCADOS[$IDM];
        }
    }
    ...
    <!- LLAMADA DE LOS ARCHIVOS NECESARIOS PARA DISEÑO Y FUNCIONES BASE DE LA VISTA -!>
        ...
        <!- FUNCIONES BASES -!>
            <script type="text/javascript">
                ...
                var mercadosProductorInicial = <?php echo json_encode($NOMBRESMERCADOSPRODUCTOR); ?>;
                var mercadosEstandarInicial = <?php echo json_encode($NOMBRESMERCADOSESTANDAR); ?>;
                ...
                function actualizarResumenMercados() {
                    var box = document.getElementById("mercados_habilitados_resumen");
                    if (!box) {
                        return;
                    }
                    var productor = document.getElementById("PRODUCTOR");
                    ...
                    var idProductor = productor ? parseInt(productor.value || 0, 10) : 0;
                    var idEstandar = estandar ? parseInt(estandar.value || 0, 10) : 0;
                    var prodIds = mercadosPorProductor[idProductor] || [];
                    var estIds = mercadosPorEstandar[idEstandar] || [];
                    var prodNombres = prodIds.map(function(idMercado) { return listaMercados[idMercado] || ("Mercado " + idMercado); });
                    var estNombres = estIds.map(function(idMercado) { return listaMercados[idMercado] || ("Mercado " + idMercado); });
                    if (prodNombres.length === 0 && mercadosProductorInicial.length > 0) {
                        prodNombres = mercadosProductorInicial;
                    }
                    if (estNombres.length === 0 && mercadosEstandarInicial.length > 0) {
                        estNombres = mercadosEstandarInicial;
                    }
                    box.innerHTML = '<strong>Mercados habilitados productor:</strong> ' + (prodNombres.length ? prodNombres.join(', ') : 'Sin mercados asignados') +
                        '<br><strong>Mercados del estándar:</strong> ' + (estNombres.length ? estNombres.join(', ') : 'Sin mercados asociados');
                }
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    actualizarResumenMercados();
                    mostrarAvisoMercados();
                    var estandar = document.getElementById('ESTANDAR');
                    if (estandar) {
                        estandar.addEventListener('change', function(){ actualizarResumenMercados(); mostrarAvisoMercados(); });
                    }
                });
                                        ...
                                        <div class="col-xxl-2 col-xl-4 col-lg-4 col-md-4 col-sm-6 col-6 col-xs-6 ">
                                            <div class="form-group">
                                                ...
                                                <label id="val_estandar" class="validacion"> </label>
                                                    <div id="mercados_habilitados_resumen" class="text-muted small mt-1"><strong>Mercados habilitados productor:</strong> <?php echo count($NOMBRESMERCADOSPRODUCTOR) ? implode(", ", $NOMBRESMERCADOSPRODUCTOR) : "Sin mercados asignados"; ?><br><strong>Mercados del estándar:</strong> <?php echo count($NOMBRESMERCADOSESTANDAR) ? implode(", ", $NOMBRESMERCADOSESTANDAR) : "Sin mercados asociados"; ?></div>
                                                    <div id="aviso_mercados_estandar" class="alert-mercados-minimal" style="display:<?php echo ($MENSAJEMERCADOS != "") ? "block" : "none"; ?>;"><?php echo $MENSAJEMERCADOS; ?></div>
                                            </div>
                                        </div>
                                        <div class="col-xxl-2 col-xl-4 col-lg-4 col-md-4 col-sm-6 col-6 col-xs-6 ">
                                            <div class="form-group">
