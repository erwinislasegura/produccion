...
$ARRAYEXISTENCIAGENERALPRODUCTOR = array();
...
if ((int)$PRODUCTOR > 0) {
    ...
    $ARRAYEXISTENCIABASE = $EXIMATERIAPRIMA_ADO->listarEximateriaprimaEmpresaPlantaTemporadaDisponible($EMPRESAS, $PLANTAS, $TEMPORADAS);
    $ARRAYEXISTENCIAGENERALPRODUCTOR = array_values(array_filter((array)$ARRAYEXISTENCIABASE, function ($rx) use ($PRODUCTOR) {
        return (int)$rx['ID_PRODUCTOR'] === (int)$PRODUCTOR;
    }));

    $TOTALEXISTENCIAPRODUCTOR = count($ARRAYEXISTENCIAGENERALPRODUCTOR);

    foreach ($ARRAYEXISTENCIAGENERALPRODUCTOR as $rex) {
        ...
        $ARRAYESTANDARX = $ERECEPCION_ADO->verEstandar($rex['ID_ESTANDAR']);
        if ($ARRAYESTANDARX) {
            $CODIGOESTANDAR = $ARRAYESTANDARX[0]['CODIGO_ESTANDAR'];
        }

        $ARRAYVESPECIESX = $VESPECIES_ADO->verVespecies($rex['ID_VESPECIES']);
        if ($ARRAYVESPECIESX) {
            $NOMBREVESPECIES = $ARRAYVESPECIESX[0]['NOMBRE_VESPECIES'];
        }

        $ARRAYTMANEJOX = $TMANEJO_ADO->verTmanejo($rex['ID_TMANEJO']);
        if ($ARRAYTMANEJOX) {
            $NOMBRETMANEJO = $ARRAYTMANEJOX[0]['NOMBRE_TMANEJO'];
        }

        $ARRAYRECEPCIONX = $RECEPCIONMP_ADO->verRecepcion2($rex['ID_RECEPCION']);
        if ($ARRAYRECEPCIONX) {
            $NUMERORECEPCION = $ARRAYRECEPCIONX[0]['NUMERO_RECEPCION'];
            $FECHARECEPCION = $ARRAYRECEPCIONX[0]['FECHA'];
            $NUMEROGUIARECEPCION = $ARRAYRECEPCIONX[0]['NUMERO_GUIA_RECEPCION'];
        }

        $GASIFICADO = "Sin Datos";
        if ($rex['GASIFICADO'] == "1") {
            $GASIFICADO = "SI";
        } else if ($rex['GASIFICADO'] == "0") {
            $GASIFICADO = "NO";
        }

        $ARRAYEXISTENCIAMODALPRODUCTOR[] = array(
            'FOLIO' => $rex['FOLIO_EXIMATERIAPRIMA'],
            'COSECHA' => isset($rex['COSECHA']) ? $rex['COSECHA'] : '',
            'DIAS' => isset($rex['DIAS']) ? $rex['DIAS'] : '',
            'CODIGOESTANDAR' => $CODIGOESTANDAR,
            'GASIFICADO' => $GASIFICADO,
            'VARIEDAD' => $NOMBREVESPECIES,
            'ENVASE' => isset($rex['ENVASE']) ? $rex['ENVASE'] : 0,
            'NETO' => isset($rex['NETO']) ? $rex['NETO'] : 0,
            'RECEPCION' => $NUMERORECEPCION,
            'FECHARECEPCION' => $FECHARECEPCION,
            'GUIA' => $NUMEROGUIARECEPCION,
            'TMANEJO' => $NOMBRETMANEJO
        );

        $TOTALENVASEEXISTENCIAPRODUCTOR += (float)$rex['ENVASE'];
        $TOTALNETOEXISTENCIAPRODUCTOR += (float)$rex['NETO'];
    }
}
                        ...
                        <?php if (isset($_GET['op'])): ?>
                            ...
                            <div class="card">
                                ...
                                <div class="card-header">
                                    <div class="form-row align-items-center">
                                        <div class="btn-group">
                                            <form class="form-one-line" data-form-layout="oneline-5" style="display:flex;flex-wrap:wrap;align-items:flex-end;gap:0.65rem;padding:0.65rem 0;border-bottom:1px solid #3a3a3a;" method="post" id="form5" name="form5">
                                                <div class="form-group">
                                                    <input type="hidden" class="form-control" placeholder="ID PROCESO" id="IDP" name="IDP" value="<?php echo $IDOP; ?>" />
                                                    <input type="hidden" class="form-control" placeholder="OP PROCESO" id="OPP" name="OPP" value="<?php echo $OP; ?>" />
                                            ...
                                            <form class="form-one-line" data-form-layout="oneline-6" style="display:flex;flex-wrap:wrap;align-items:flex-end;gap:0.65rem;padding:0.65rem 0;border-bottom:1px solid #3a3a3a;" method="post" id="form6" name="form6">
                                                <div class="form-group">
                                                    <input type="hidden" class="form-control" placeholder="ID PROCESO" id="IDP" name="IDP" value="<?php echo $IDOP; ?>" />
                                                    <input type="hidden" class="form-control" placeholder="OP PROCESO" id="OPP" name="OPP" value="<?php echo $OP; ?>" />
                                                    <input type="hidden" class="form-control" placeholder="URL PROCESO" id="URLP" name="URLP" value="registroProceso" />
                                                ...
                                                <?php if ($ARRAYDEXPORTACIONPORPROCESO) { ?>
                                                    ...
                                                    <tr class="text-center">
                                                        <td>
                                                            ...
                                                            <td class="text-center">
                                                                <form class="form-one-line" data-form-layout="oneline-7" style="display:flex;flex-wrap:wrap;align-items:flex-end;gap:0.65rem;padding:0.65rem 0;border-bottom:1px solid #3a3a3a;" method="post" id="form3" id="form3">
                                                ...
                                                <?php if ($ARRATDINDUSTRIALPORPROCESO) { ?>
                                                    <?php foreach ($ARRATDINDUSTRIALPORPROCESO as $r) : ?>
                                                        ...
                                                        <tr class="text-center">
                                                            ...
                                                            <td class="text-center">
                                                                <form class="form-one-line" data-form-layout="oneline-8" style="display:flex;flex-wrap:wrap;align-items:flex-end;gap:0.65rem;padding:0.65rem 0;border-bottom:1px solid #3a3a3a;" method="post" id="form4" id="form4">
            ...
            <div class="modal-body">
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-minimal" style="width:100%;">
                        ...
                        <tbody>
                            <?php if ($ARRAYEXISTENCIAMODALPRODUCTOR) { ?>
                                <?php foreach ($ARRAYEXISTENCIAMODALPRODUCTOR as $rx) { ?>
                                    <tr>
                                        <td><?php echo $rx['FOLIO']; ?></td>
                                        <td><?php echo $rx['COSECHA']; ?></td>
                                        <td><?php echo $rx['DIAS']; ?></td>
                                        <td><?php echo $rx['CODIGOESTANDAR']; ?></td>
                                        <td><?php echo $rx['GASIFICADO']; ?></td>
                                        <td><?php echo $rx['VARIEDAD']; ?></td>
                                        <td><?php echo $rx['ENVASE']; ?></td>
                                        <td><?php echo $rx['NETO']; ?></td>
                                        <td><?php echo $rx['RECEPCION']; ?></td>
                                        <td><?php echo $rx['FECHARECEPCION']; ?></td>
                                        <td><?php echo $rx['GUIA']; ?></td>
                                        <td><?php echo $rx['TMANEJO']; ?></td>
                                    </tr>
                                <?php } ?>
                            <?php } else { ?>
                                <tr>
                                    <td colspan="12" class="text-center">No hay existencia activa para este productor.</td>
                                </tr>
                            <?php } ?>
                        </tbody>
                        <tfoot>
                            <tr>
                                ...
                                <th><?php echo number_format($TOTALENVASEEXISTENCIAPRODUCTOR, 0, ',', '.'); ?></th>
                                <th><?php echo number_format($TOTALNETOEXISTENCIAPRODUCTOR, 2, ',', '.'); ?></th>
                                <th colspan="4"><?php echo $TOTALEXISTENCIAPRODUCTOR; ?> registros</th>
                            </tr>
                        </tfoot>
                    </table>
                </div>
